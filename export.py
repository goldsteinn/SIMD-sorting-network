...
def shuffle_mask_impl(lane_size, ele_per_lane, slot_bits, offset, perm):
    ...
    for i in range(0, N_lanes):
        lane_mask = int(0)
        lower_bound = N - ((i + 1) * lane_size + offset)
...
def shuffle_mask_impl_epi16(lane_size, ele_per_lane, slot_bits, offset, perm):
    ...
    for i in range(0, N_lanes):
        ...
        for j in range(0, ele_per_lane):
            ...
            err_assert(idx < 4, "invalid idx, p[{}]({}) -> [{} - {}]".format(i * lane_size + j + offset, p, lower_bound, upper_bound))
            slot = slot_bits * ((ele_per_lane - 1) - j)
...
class SIMD_Shuffle_As_Epi32(SIMD_Instruction):
    ...
    def shuffle_mask(self):
        ...
        if T_size < 4:
            ...
            scaled_perm = scale_perm(T_size, 4, self.perm)
            if in_same_lanes(16, T_size, scaled_perm) is False:
                return int(-1)
    ...
    def shuffle_mask_lt_T(self):
        ...
        scaled_perm = scale_perm(T_size, 4, self.perm)
        if in_same_lanes(16, 4, scaled_perm) is False:
            return int(-1)

        return shuffle_mask_impl(4, 4, 2, 0, scaled_perm)
...
class SIMD_Shuffle_As_Epi16(SIMD_Instruction):
    ...
    def shuffle_mask(self):
        ...
        scaled_perm = scale_perm(T_size, 2, self.perm)
        if in_same_lanes(8, 2, scaled_perm) is False:
            return int(-1)
    ...
    def build_shuffle_mask(self, new_perm):
        shuffle_mask_lo = shuffle_mask_impl_epi16(8, 4, 2, 4, new_perm)
        shuffle_mask_hi = shuffle_mask_impl_epi16(8, 4, 2, 0, new_perm)
        print("{} -> {}, {}".format(str(new_perm), str(hex(shuffle_mask_lo)), str(hex(shuffle_mask_hi))))
        ...
        return self.has_support() and self.match_sort_type(
            match_info.sort_type) and self.match_simd_type(
                match_info.simd_type) and in_same_lanes(
                    16, self.T_size, scale_perm(self.T_size, 1, self.perm))
