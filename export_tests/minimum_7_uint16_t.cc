...
#define N 7
#define SORT_NAME minimum_7_uint16_t

#ifndef _SIMD_SORT_minimum_7_uint16_t_H_
#define _SIMD_SORT_minimum_7_uint16_t_H_
...
/* SIMD Sort */
__m128i __attribute__((const)) minimum_7_uint16_t_vec(__m128i v) {

/* Pairs: ([7,7], [0,6], [4,5], [2,3], [1,1]) */
/* Perm:  ( 7,  0,  4,  5,  2,  3,  1,  6) */
__m128i perm0 = _mm_shuffle_epi8(v, _mm_set_epi8(15, 14, 1, 0, 9, 8, 11, 10, 5, 4, 7, 6, 3, 2, 13, 12));
...
__m128i perm1 = _mm_shuffle_epi8(v0, _mm_set_epi8(15, 14, 7, 6, 11, 10, 3, 2, 13, 12, 1, 0, 9, 8, 5, 4));
__m128i min1 = _mm_min_epu16(v0, perm1);
__m128i max1 = _mm_max_epu16(v0, perm1);
__m128i v1 = _mm_blend_epi16(max1, min1, 0xb);

/* Pairs: ([7,7], [6,6], [2,5], [3,4], [0,1]) */
/* Perm:  ( 7,  6,  2,  3,  4,  5,  0,  1) */
__m128i perm2 = _mm_shuffle_epi8(v1, _mm_set_epi8(15, 14, 13, 12, 5, 4, 7, 6, 9, 8, 11, 10, 1, 0, 3, 2));
...
__m128i perm3 = _mm_shufflehi_epi16(_mm_shufflelo_epi16(v2, 0xd8), 0xc6);
__m128i min3 = _mm_min_epu16(v2, perm3);
__m128i max3 = _mm_max_epu16(v2, perm3);
__m128i v3 = _mm_blend_epi16(max3, min3, 0x12);

/* Pairs: ([7,7], [6,6], [4,5], [2,3], [1,1], [0,0]) */
/* Perm:  ( 7,  6,  4,  5,  2,  3,  1,  0) */
__m128i perm4 = _mm_shufflehi_epi16(_mm_shufflelo_epi16(v3, 0xb4), 0xe1);
__m128i min4 = _mm_min_epu16(v3, perm4);
__m128i max4 = _mm_max_epu16(v3, perm4);
__m128i v4 = _mm_blend_epi16(max4, min4, 0x14);
...
void inline __attribute__((always_inline)) minimum_7_uint16_t(uint16_t * const arr) {

__m128i _tmp0 = _mm_set1_epi16(uint16_t(0xffff));
__m128i v = _mm_mask_loadu_epi16(_tmp0, 0x7f, arr);
fill_works(v);
v = minimum_7_uint16_t_vec(v);
